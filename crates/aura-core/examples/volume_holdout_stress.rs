//! Held-out perturbation stress suite.
//!
//! Unlike `volume_scaling` and `volume_combo_stress`, this runner uses
//! perturbations that are intentionally not part of the tuned baseline set:
//! fullwidth Unicode, mixed-script homoglyphs, combining marks, interstitial
//! separators and whitespace noise.

mod common;

use aura_core::scenario_packs::pack_index;
use aura_core::scenario_packs::volume::{perturb_pack_holdout, HoldoutPerturbationKind};
use aura_core::{
...
fn main() {
    ...
    let issue_limit = std::env::var("AURA_HOLDOUT_ISSUES")
        .ok()
    ...
    let worker_count = scenario_worker_count("AURA_HOLDOUT_THREADS");

    println!(
        "AURA held-out perturbation stress suite ({} packs x {} holdout perturbations, long-context excluded)\n",
        packs.len(),
        kinds.len()
    );
    ...
    for (pack_idx, pack) in packs.iter().enumerate() {
        ...
        for case in perturb_pack_holdout(&base, kinds) {
    ...
    let mut kind_outcomes: BTreeMap<String, Outcome> = BTreeMap::new();
    let mut issues = Vec::new();

    for ((case, run), meta) in cases.iter().zip(batch.runs.iter()).zip(metas.iter()) {
        let perturbation = perturbation_label(case);
    ...
    println!(
        "{:<28} {:>8} {:>8} {:>9}",
        "holdout", "pos%", "fp%", "variants"
    );
    println!("{}", "-".repeat(56));
    for (kind, outcome) in &kind_outcomes {
        println!(
            "{:<28} {:>7.1}% {:>7.1}% {:>9}",
            kind,
            outcome.pos_percent(),
    ...
    println!();
    if issues.is_empty() {
        println!("No misses or false positives in held-out stress.");
    } else {
        println!("First {} issues:", issue_limit.min(issues.len()));
        println!(
            "{:<5} {:<22} {:<28} {:<46} {:>6} {:>6} text",
            "kind", "pack", "holdout", "case", "peak", "thr"
        );
        println!("{}", "-".repeat(139));
    ...
    let report = build_volume_report(
        ...
        kind_outcomes
            .iter()
            .map(|(kind, outcome)| MetricRow::new(kind.clone(), outcome_report(*outcome)))
            .collect(),
        issues
            .iter()
            .map(|issue| {
    ...
    finalize_volume_report("AURA_HOLDOUT", &report);
}
...
fn perturbation_label(case: &ScenarioCase) -> String {
    case.name
        .rsplit_once("__")
