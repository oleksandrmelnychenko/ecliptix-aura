...
use aura_military::temporal_review_packet::generate_embedded_temporal_blind_review_materials;
use serde::Serialize;
...
fn usage() -> &'static str {
    "usage: cargo run -p aura-military --features evaluation --example temporal_blind_review_packet -- --packet-id TOKEN --blinding-key PATH --packet-output PATH --coordinator-map-output PATH --review-template-output PATH"
}
...
fn run() -> Result<i32, String> {
    ...
    let materials =
        generate_embedded_temporal_blind_review_materials(&args.packet_id, &blinding_key)
...
fn validate_distinct_paths(args: &CliArgs) -> Result<(), String> {
    let protected = normalized_path(&args.blinding_key)?;
    let outputs = [
        normalized_path(&args.packet_output)?,
        normalized_path(&args.coordinator_map_output)?,
        normalized_path(&args.review_template_output)?,
    ];
    if outputs.contains(&protected) {
        return Err("an output path must not overwrite the blinding key".to_string());
    }
