//! Per-case diagnostic for DOXXING pack.

use aura_core::scenario_packs::doxxing::{
    adversarial_doxxing_scenarios, canonical_doxxing_scenarios, false_positive_doxxing_scenarios,
    long_context_doxxing_scenarios,
};
...
fn main() {
    ...
    let mut base = canonical_doxxing_scenarios();
    base.extend(adversarial_doxxing_scenarios());
    base.extend(long_context_doxxing_scenarios());
    base.extend(false_positive_doxxing_scenarios());

    let all = if std::env::var_os("DOX_VOLUME").is_some() {
    ...
    for case in &all {
        ...
        for (idx, step_result) in run.step_results.iter().enumerate() {
            let score = predicted_score_for_threat(step_result, ThreatType::Doxxing);
            if score > peak {
                peak = score;
            }
        ...
        let label = match case.primary_threat {
            Some(ThreatType::Doxxing) => "POS",
            Some(_) => "POS-other",
            None => "NEG",
        };
        let detected = first_hit.is_some();
        if case.primary_threat == Some(ThreatType::Doxxing) {
            pos_total += 1;
            if detected {
                pos_hits += 1;
            }
