...
"""Issue trusted-time receipts for independent recomputation evidence.

This module is deliberately a closed profile over
``domain_result_timestamp_adapter``.  All nonce, message-imprint, RFC 3161,
PKIX-path, historical-CRL, immutable-input, private-key, and atomic-output
checks remain owned by that hardened implementation.  Only the accepted schema,
subject allowlist, and Ed25519 signature domain differ from result receipts.
"""
...
RECOMPUTATION_TIMESTAMP_PROFILE = adapter_support.TimestampReceiptProfile(
    ...
    cli_label="domain-recomputation",
)
...
def signing_payload(claims: dict, key_id: str) -> bytes:
    """Return the exact recomputation timestamp signature payload."""

    return adapter_support.signing_payload(
        claims,
        key_id,
        profile=RECOMPUTATION_TIMESTAMP_PROFILE,
    )
...
def sign_claims(
    ...
    *,
    ...
    """Sign recomputation timestamp claims with the configured verifier key."""
...
def create_trusted_timestamp_receipt(
    *,
    ...
    """Verify original RFC 3161 evidence and issue the separate receipt type."""
...
def parse_args():
    ...
    return adapter_support.parse_args(RECOMPUTATION_TIMESTAMP_PROFILE)


def main() -> int:
    return adapter_support.main(RECOMPUTATION_TIMESTAMP_PROFILE)
