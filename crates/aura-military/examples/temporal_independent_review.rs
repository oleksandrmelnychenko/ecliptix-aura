...
fn usage() -> &'static str {
    "usage: cargo run -p aura-military --features evaluation --example temporal_independent_review -- --review-bundle PATH [--output PATH] [--require-pass]"
}
...
fn run() -> Result<i32, String> {
    ...
    let raw = fs::read_to_string(&args.review_bundle).map_err(|err| {
        format!(
            "read temporal independent-review bundle {}: {err}",
            args.review_bundle.display()
        )
    })?;
    let report = evaluate_embedded_temporal_review(&raw).map_err(|err| err.to_string())?;
