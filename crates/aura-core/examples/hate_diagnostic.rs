//! Per-case diagnostic for HATE_SPEECH pack.

use aura_core::scenario_packs::hate_speech::{
    adversarial_hate_speech_scenarios, canonical_hate_speech_scenarios,
    false_positive_hate_speech_scenarios, long_context_hate_speech_scenarios,
};
...
fn main() {
    ...
    let mut base = canonical_hate_speech_scenarios();
    base.extend(adversarial_hate_speech_scenarios());
    base.extend(long_context_hate_speech_scenarios());
    base.extend(false_positive_hate_speech_scenarios());

    let all = if std::env::var_os("HATE_VOLUME").is_some() {
    ...
    for case in &all {
        ...
        for (idx, step_result) in run.step_results.iter().enumerate() {
            let score = predicted_score_for_threat(step_result, ThreatType::HateSpeech);
            if score > peak {
                peak = score;
            }
        ...
        let label = match case.primary_threat {
            Some(ThreatType::HateSpeech) => "POS",
            Some(_) => "POS-other",
            None => "NEG",
        };
        let detected = first_hit.is_some();
        if case.primary_threat == Some(ThreatType::HateSpeech) {
            pos_total += 1;
            if detected {
                pos_hits += 1;
            }
