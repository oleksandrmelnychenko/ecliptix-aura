//! Per-case diagnostic for PHISHING pack.

use aura_core::scenario_packs::phishing::{
    adversarial_phishing_scenarios, canonical_phishing_scenarios,
    false_positive_phishing_scenarios, long_context_phishing_scenarios,
};
...
fn main() {
    ...
    let mut base = canonical_phishing_scenarios();
    base.extend(adversarial_phishing_scenarios());
    base.extend(long_context_phishing_scenarios());
    base.extend(false_positive_phishing_scenarios());

    let all = if std::env::var_os("PHISH_VOLUME").is_some() {
    ...
    for case in &all {
        ...
        for (idx, step_result) in run.step_results.iter().enumerate() {
            let score = predicted_score_for_threat(step_result, ThreatType::Phishing);
            if score > peak {
                peak = score;
            }
        ...
        let label = match case.primary_threat {
            Some(ThreatType::Phishing) => "POS",
            Some(_) => "POS-other",
            None => "NEG",
        };
        let detected = first_hit.is_some();
        if case.primary_threat == Some(ThreatType::Phishing) {
            pos_total += 1;
            if detected {
                pos_hits += 1;
            }
        ...
        println!(
            ...
            fired_step_text
                ...
                .or_else(|| {
                    case.steps
                        .iter()
                        .find(|step| step.observed_threats.contains(&ThreatType::Phishing))
