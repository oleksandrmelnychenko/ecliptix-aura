//! Per-case diagnostic for MILITARY_SOCIAL_ENG pack.

use aura_core::scenario_packs::military_social_eng::{
    adversarial_military_social_eng_scenarios, canonical_military_social_eng_scenarios,
    false_positive_military_social_eng_scenarios, long_context_military_social_eng_scenarios,
};
...
fn main() {
    ...
    let all = if std::env::var_os("MSE_VOLUME").is_some() {
        perturb_pack(&all, PerturbationKind::default_set())
    } else {
        all
    };
    ...
    for case in &all {
        ...
        for (idx, step_result) in run.step_results.iter().enumerate() {
            let score = predicted_score_for_threat(step_result, ThreatType::MilitarySocialEng);
            if score > peak {
                peak = score;
            }
        ...
        let label = match case.primary_threat {
            Some(ThreatType::MilitarySocialEng) => "POS",
            Some(_) => "POS-other",
            None => "NEG",
        };
        let detected = first_hit.is_some();
        if case.primary_threat == Some(ThreatType::MilitarySocialEng) {
            pos_total += 1;
            if detected {
                pos_hits += 1;
            }
