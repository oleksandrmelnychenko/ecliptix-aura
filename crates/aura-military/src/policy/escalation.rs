...
pub fn needs_priority_escalation(signals: &[DomainSignal]) -> bool {
    needs_priority_escalation_with_priority(signals, 90)
}

pub fn needs_priority_escalation_with_priority(signals: &[DomainSignal], min_priority: u8) -> bool {
...
#[cfg(test)]
mod tests {
    use super::needs_priority_escalation_with_priority;
    use aura_domain::DomainSignal;
    ...
    fn escalates_on_priority_threshold() {
        let signals = vec![signal(Some(90), Some("high"))];
        assert!(needs_priority_escalation_with_priority(&signals, 90));
    }

    #[test]
    fn escalates_on_critical_even_below_priority_threshold() {
        let signals = vec![signal(Some(20), Some("critical"))];
        assert!(needs_priority_escalation_with_priority(&signals, 90));
    }

    #[test]
    fn does_not_escalate_below_threshold_without_critical() {
        let signals = vec![signal(Some(80), Some("high"))];
        assert!(!needs_priority_escalation_with_priority(&signals, 90));
    }
}
