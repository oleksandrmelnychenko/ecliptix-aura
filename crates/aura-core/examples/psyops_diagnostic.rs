//! Per-case diagnostic for PSYOPS pack.

use aura_core::scenario_packs::psyops::{
    adversarial_psyops_scenarios, canonical_psyops_scenarios, false_positive_psyops_scenarios,
    long_context_psyops_scenarios,
};
...
fn main() {
    ...
    let mut base = canonical_psyops_scenarios();
    base.extend(adversarial_psyops_scenarios());
    base.extend(long_context_psyops_scenarios());
    base.extend(false_positive_psyops_scenarios());

    let all = if std::env::var_os("PSYOPS_VOLUME").is_some() {
    ...
    for case in &all {
        ...
        for (idx, step_result) in run.step_results.iter().enumerate() {
            let score = predicted_score_for_threat(step_result, ThreatType::Psyops);
            if score > peak {
                peak = score;
            }
        ...
        let label = match case.primary_threat {
            Some(ThreatType::Psyops) => "POS",
            Some(_) => "POS-other",
            None => "NEG",
        };
        let detected = first_hit.is_some();
        if case.primary_threat == Some(ThreatType::Psyops) {
            pos_total += 1;
            if detected {
                pos_hits += 1;
            }
        ...
        println!(
            ...
            fired_step_text
                ...
                .or_else(|| {
                    case.steps
                        .iter()
                        .find(|step| step.observed_threats.contains(&ThreatType::Psyops))
