//! Per-case diagnostic for EXPLICIT_THREAT pack.

use aura_core::scenario_packs::explicit_threat::{
    adversarial_explicit_threat_scenarios, canonical_explicit_threat_scenarios,
    false_positive_explicit_threat_scenarios, long_context_explicit_threat_scenarios,
};
...
fn main() {
    ...
    let mut base = canonical_explicit_threat_scenarios();
    base.extend(adversarial_explicit_threat_scenarios());
    base.extend(long_context_explicit_threat_scenarios());
    base.extend(false_positive_explicit_threat_scenarios());

    let all = if std::env::var_os("THREAT_VOLUME").is_some() {
    ...
    for case in &all {
        ...
        for (idx, step_result) in run.step_results.iter().enumerate() {
            let target = case.primary_threat.unwrap_or(ThreatType::Threat);
            let score = predicted_score_for_threat(step_result, target).max(
                predicted_score_for_threat(step_result, ThreatType::Explicit),
            );
            if score > peak {
                peak = score;
            }
        ...
        let label = match case.primary_threat {
            Some(ThreatType::Threat) => "POS-thr",
            Some(ThreatType::Explicit) => "POS-exp",
            Some(_) => "POS-oth",
            None => "NEG",
        };
        ...
        println!(
            ...
            fired_step_text
                ...
                .or_else(|| {
                    case.steps
                        .iter()
                        .find(|step| !step.observed_threats.is_empty())
