//! Per-case diagnostic for COORDINATE_LEAK pack.

use aura_core::scenario_packs::coordinate_leak::{
    adversarial_coordinate_leak_scenarios, canonical_coordinate_leak_scenarios,
    false_positive_coordinate_leak_scenarios, long_context_coordinate_leak_scenarios,
};
...
fn main() {
    ...
    let mut base = canonical_coordinate_leak_scenarios();
    base.extend(adversarial_coordinate_leak_scenarios());
    base.extend(long_context_coordinate_leak_scenarios());
    base.extend(false_positive_coordinate_leak_scenarios());

    let all = if std::env::var_os("COORD_VOLUME").is_some() {
    ...
    for case in &all {
        ...
        for (idx, step_result) in run.step_results.iter().enumerate() {
            let score = predicted_score_for_threat(step_result, ThreatType::CoordinateLeak);
            if score > peak {
                peak = score;
            }
        ...
        let label = match case.primary_threat {
            Some(ThreatType::CoordinateLeak) => "POS",
            Some(_) => "POS-other",
            None => "NEG",
        };
        let detected = first_hit.is_some();
        if case.primary_threat == Some(ThreatType::CoordinateLeak) {
            pos_total += 1;
            if detected {
                pos_hits += 1;
            }
